...
# argument 1: enterprise name or "continue" to continue from where you left off
...
# 
# Check if $1 is "continue" then just run enableFeatures otherwise run all the steps
if [ "$1" == "continue" ]; then
    echo "Continuing from where it left off"
    enableFeatures
else
    # if there are less then  3 arguments, print the usage and exit
    if [ $# -lt 3 ]; then
        echo "Usage: $0 <enterpriseName> <features> <ghesServerUrl>"
        echo "Example: $0 myenterprise code-scanning,secret-scanning https://github.mycompany.com"
        exit 1
    fi
    configure $1 $2 $3
    enableFeatures
